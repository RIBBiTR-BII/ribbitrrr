...
scrape_amphibiaweb <- function(sci_nam) {
  ...
  process_name <- function(name_submitted) {
    ...
    gna_results = gna_verifier(name_submitted, data_sources = 118, capitalize = TRUE)
    name_matched = as.character(gna_results[1, 'matchedName'])
    ...
    tryCatch({
      ...
      extract_value <- function(xpath) {
        value <- xml_text(xml_find_first(parsed_xml, xpath))
        ifelse(length(value) == 0, NA, trimws(value))
      }

      tibble(
        name_submitted = name_submitted,
        scientific_name = name_matched,
        record_id = gna_results[1, 'recordId'],
        sort_score = gna_results[1, 'sortScore'],
        matched_name_id = gna_results[1, 'matchedNameID'],
      ...
      return(tibble(
        name_submitted = name_submitted, scientific_name = NA,
        record_id = NA, sort_score = NA, matched_name_id = NA,
        amphib_id = NA, ordr = NA, family = NA, subfamily = NA,
        genus = NA, species = NA, clade = NA, common_name = NA,
        url = NA
      ))
    })
  }
